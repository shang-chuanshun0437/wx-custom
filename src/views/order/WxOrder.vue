    ...
    <div class="front-order-list-content">
      <div class="panel-heading">
        <el-breadcrumb separator-class="el-icon-arrow-right">
          ...
          <el-breadcrumb-item style="font-size: 17px;">扫码订单</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
      ...
      <div class="front-order-list-table">
        <el-table v-loading="loading" :data="list" style="width:98%;left: 1%" @row-click="clickRow" border stripe ref="moviesTable">
          ...
          <el-table-column width="80" prop="tableId" label="餐桌编号" align="center"></el-table-column>
  ...
  methods: {
    refresh() {
      ...
      var param = Object.assign({}, {userPhone: user.userPhone , token: user.token ,storeId: this.storeId ,
        ...
        endTime: this.filters.endTime ,source: 1, orderStatus: 2,currentPage: this.currentPage });
    ...
    orderInfo(row) {
      this.$router.push({path:"/user/store/list/storeInfo/frontDeskOrder/orderInfo",query:{orderId: row.orderId,
          tableId: row.tableId,amount: row.amount,vipAmount: row.vipAmount,realAmount: row.realAmount,orderStatus: row.orderStatus}});
    },
